...
function AttributeForm({
  ...
  const selectDicts = catalogs.reduce((acc, catalog) => {
    acc[catalog.nick] = {
      label: catalog.name,
      value: catalog.nick,
    }
  ...
  return (
    <Card bg="white" p={2} width={width}>
      {form.getFieldValue('name') ? (
        <Heading tag="h5" mb={3}>
          Атрибут &laquo;{form.getFieldValue('name')}&raquo;
        </Heading>
      ) : (
        <Heading tag="h5" mb={3}>
          Новый атрибут
      ...
      <Scrollbars style={{ height: contentHeight }} autoHide>
        <Box py={2} px={3}>
          ...
          >
            <Select options={typeOptions} isDisabled={attribute} />
          </FormItem>
          {type === 'link' && (
            <FormItem required mb={2} name="link" label="Справочник" form={form}>
              <Select options={Object.values(selectDicts)} isDisabled={attribute} />
            </FormItem>
          )}
          <FormItem
            mb={2}
          ...
          <Flex mb={1}>
            <FormItem mb={0} name="array" form={form}>
              <Toggle />
            </FormItem>
            <Text ml={2} mt={2}>
              &mdash; Множественное
          ...
          <Flex mb={1}>
            <FormItem mb={0} name="required" form={form}>
              <Toggle />
            </FormItem>
            <Text ml={2} mt={2}>
              &mdash; Обязательное
          ...
          <Flex mb={1}>
            <FormItem mb={0} name="unique" form={form}>
              <Toggle />
            </FormItem>
            <Text ml={2} mt={2}>
              &mdash; Уникальное
          ...
          {form.getFieldDecorator('orders', { initialValue: attributes ? attributes.length : 0 })}
        </Box>
      </Scrollbars>
  ...
  Form.create({
    mapPropsToFields(props) {
      ...
      return Object.keys(data).reduce((acc, key) => {
        switch (key) {
          case 'type':
            acc[key] = Form.createFormField({ value: typeOptions.find(type => type.value === data[key]) })
            break
          case 'link':
            acc[key] = Form.createFormField({ value: selectDicts[data[key]] })
            break
          default:
