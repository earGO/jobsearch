...
function SortableColumn({ nick, catalog, ...props }) {
  const dispatch = useDispatch()
  const activeSort = useSelector(selectors.sort)

  const handleSort = (order, column) => () => {
    // console.log(catalog, nick, order, column, props)
    // saveState
    dispatch(actions.sort({ order, column }))
  }
  const isActive = order => activeSort.column === nick && order === activeSort.order

  return (
    <Flex {...props} flexDirection="column">
      <SortIcon up isActive={isActive('asc')} onClick={handleSort('asc', nick)} />
      <SortIcon isActive={isActive('desc')} onClick={handleSort('desc', nick)} />
    </Flex>
  )
}
