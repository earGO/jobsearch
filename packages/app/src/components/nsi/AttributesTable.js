      ...
      <Table rowKey="nick" autoHeight rowHeight={62} data={attributes}>
        ...
        <Table.Column flexGrow={1}>
          ...
          <TableCell dataKey="name">
            {row => {
              return (
                <React.Fragment>
                  <Text truncated onClick={() => handleEdit(row)} style={{ cursor: 'pointer' }}>
                    <Text>
        ...
        <Table.Column width={130}>
          ...
          <TableCell dataKey="array" align="center">
            {row => <Toggle checked={row.array} onChange={handleItemChange('array', row.nick)} />}
          </TableCell>
        </Table.Column>
        <Table.Column width={130}>
          <HeaderCell align="center">Обязательное</HeaderCell>
          <TableCell dataKey="required" align="center">
            {row => <Toggle checked={row.required} onChange={handleItemChange('required', row.nick)} />}
          </TableCell>
        </Table.Column>
        <Table.Column width={100}>
          <HeaderCell align="center">Уникальное</HeaderCell>
          <TableCell dataKey="unique" align="center">
            {row => <Toggle checked={row.unique} onChange={handleItemChange('unique', row.nick)} />}
          </TableCell>
        </Table.Column>
        <Table.Column align="center" width={80}>
          <HeaderCell>Удалено</HeaderCell>
          <TableCell dataKey="deleted" align="center">
            {row => <Toggle checked={row.deleted} onChange={handleItemChange('deleted', row.nick)} />}
          </TableCell>
        </Table.Column>
      </Table>
    </Box>
