...
import * as nsi from '../../../services/nsi'
...
export const filteredCatalogs = createSelector(
  ...
  (catalogs, query) =>
    Object.values(catalogs || {}).filter(catalog =>
...
export const filteredAttributes = createSelector(
  ...
  attributes => Object.values(attributes).sort((a, b) => a.orders - b.orders),
)

export const filteredElements = createSelector(
  [currentCatalogElements, currentCatalogAttributes, elementsSearchQuery, sort],
  (elements = {}, attributes = {}, searchQuery, { column, order }) => {
    let result = Object.values(elements)
    const SORTERS = {
...
export const groupedCatalogs = createSelector(
  filteredCatalogs,
  catalogs => R.groupBy(catalog => catalog && catalog.sysName)(Object.values(catalogs)),
)
